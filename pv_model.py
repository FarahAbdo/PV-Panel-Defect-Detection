...
def main():
    ...
    st.title("PV Panel Defect Detection")
    st.markdown("""
    Analyze photovoltaic panels to detect defects using trained model to identify 5 different types of PV panel defects.
    """)
    ...
    input_method = st.radio("Choose input method:", ["Upload an image", "Take a photo with camera"])
    
    input_file = None
    
    if input_method == "Upload an image":
        # File uploader for image
        input_file = st.file_uploader("Upload a PV panel image", type=["jpg", "jpeg", "png"])
    else:
        # Camera input option (only works on devices with cameras)
        input_file = st.camera_input("Take a photo of a PV panel")
    ...
    # Cleanup custom model file if it exists
    if custom_model and 'model_path' in locals() and os.path.exists(model_path) and model_path.startswith(tempfile.gettempdir()):
        try:
            ...
            if debug_mode:
                st.warning(f"Could not delete temporary model file: {str(e)}")
